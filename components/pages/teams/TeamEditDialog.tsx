...
export function TeamEditDialog({ open, onOpenChange, teamId, initialName, initialDescription, vendorId }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError(null)
    try {
      await mutateAsync({ name, description: description || undefined, vendorId })
      onOpenChange(false)
    } catch (err) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="w-full max-w-md rounded bg-white p-4 shadow">
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-lg font-semibold">Chỉnh sửa team</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            <button type="submit" disabled={isPending} className="px-3 py-2 rounded bg-black text-white disabled:opacity-50">
