    ...
    return (
      <div className="space-y-6">
        ...
        <div className="space-y-4">
          {[1, 2, 3].map(i => (
            <div key={i} className="h-24 w-full bg-slate-50 animate-pulse rounded-2xl border border-slate-100" />
          ))}
        </div>
      </div>
    )
      ...
      <div className="space-y-4">
        {sessions.map((session) => (
          <Card
            key={session.id}
            className={cn(
              "rounded-3xl border-slate-100 bg-white transition-all duration-300 shadow-sm hover:shadow-md",
              session.current && "border-slate-900/10 ring-1 ring-slate-100 flex-1 overflow-visible"
            )}
          >
            <CardContent className="p-6 md:p-8 flex items-center justify-between gap-6">
              <div className="flex items-center gap-6">
                <div className={cn(
                  ...
                  session.current ? "bg-slate-900 text-white shadow-xl shadow-slate-200" : "bg-slate-50 text-slate-400 border border-slate-100"
                )}>
                ...
                <div className="space-y-1">
                  <div className="flex items-center gap-3">
                    <span className="font-bold text-slate-900 text-lg">{session.device}</span>
                    {session.current && (
              ...
              {!session.current && (
                <Button
                  ...
                  className="rounded-xl h-10 px-4 text-xs font-bold uppercase tracking-widest text-slate-400 hover:text-rose-600 hover:bg-rose-50 transition-all"
                >
