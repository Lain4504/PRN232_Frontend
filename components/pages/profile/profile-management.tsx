    ...
    <div className="max-w-[1440px] mx-auto font-fira-sans">
      <div className="space-y-10 p-6 lg:p-10 bg-background">
            ...
            {profiles.length > 0 ? (
              <div className="grid grid-cols-1 gap-4">
                {profiles.map((profile) => {
                  const profileType = profile.profileType === 'Free' ? 'personal' : 'business';
                  return (
                    <Card key={profile.id} className="group relative bg-card/40 backdrop-blur-xl border-border/40 hover:border-primary/50 rounded-[2.5rem] transition-all duration-300 shadow-xl shadow-black/5 overflow-hidden">
              ...
              <Card className="border-4 border-dashed border-border/40 bg-card/20 backdrop-blur-sm rounded-[3rem] p-24 text-center space-y-8">
                <div className="mx-auto h-24 w-24 rounded-[2.5rem] bg-muted/20 flex items-center justify-center">
        ...
        {(viewingProfileId || editingProfileId || creatingProfile) && (
          isMobile ? (
            ...
            <Dialog open={!!(viewingProfileId || editingProfileId || creatingProfile)} onOpenChange={handleCloseModal}>
              <DialogContent className="max-w-2xl font-fira-sans rounded-[3rem] border-border/40 bg-background/95 backdrop-blur-3xl p-12 shadow-[0_0_100px_rgba(0,0,0,0.3)]">
        ...
        <AlertDialog open={!!deleteProfileId} onOpenChange={() => setDeleteProfileId(null)}>
          <AlertDialogContent className="max-w-md font-fira-sans rounded-[2.5rem] border-border/40 bg-background/95 backdrop-blur-3xl p-10 shadow-2xl">
  ...
  return (
    <div className="space-y-10">
      <div className="flex flex-col sm:flex-row items-center sm:items-start gap-10">
        <div className="relative h-32 w-32 rounded-[2.5rem] overflow-hidden border-2 border-primary/20 shadow-2xl shrink-0">
...
function AvatarPreview({
  ...
  return (
    <div className="relative h-20 w-20 rounded-[1.8rem] overflow-hidden border border-border/40 shadow-inner shrink-0 cursor-pointer group hover:scale-105 transition-transform duration-500">
    ...
    <div className="space-y-10">
      <FormField label="Identity Sector Type" required>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
          <div
            className={`p-6 border-2 rounded-[1.8rem] cursor-pointer transition-all duration-300 group ${formData.profile_type === 'personal'
          ...
          <div
            className={`p-6 border-2 rounded-[1.8rem] cursor-pointer transition-all duration-300 group ${formData.profile_type === 'business'
                ? 'border-primary bg-primary/5 shadow-xl shadow-primary/5'
                : 'border-border/40 hover:border-primary/40 hover:bg-muted/30'
              }`}
            onClick={() => setFormData({
      ...
      >
        <Textarea
          ...
          className="bg-muted/20 border-border/40 hover:border-primary/40 transition-all rounded-[1.8rem] min-h-[120px] font-medium text-sm p-6 leading-relaxed tracking-tight"
        />
