...
import { Loader2, Package, Upload, Boxes } from "lucide-react";
...
export function TeamProductForm({ mode, product, defaultBrandId, teamId, onSuccess, onCancel }: TeamProductFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.brand_id) return toast.error('Vui lòng chọn thương hiệu');
  ...
  if (teamBrands.length === 0) {
    return (
      <div className="py-12 px-6 text-center border-2 border-dashed rounded-lg bg-slate-50/50">
        <Boxes className="size-12 text-slate-300 mx-auto mb-4" />
        <h3 className="text-sm font-bold text-slate-900 mb-1">Đội ngũ chưa có thương hiệu</h3>
        <p className="text-xs text-slate-500 max-w-xs mx-auto">
          Vui lòng gán thương hiệu cho đội ngũ này trước khi tạo sản phẩm.
        </p>
      </div>
    );
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 pb-4">
      <div className="space-y-2">
        ...
        >
          <SelectTrigger>
            <SelectValue placeholder="Chọn thương hiệu nội bộ..." />
          </SelectTrigger>
          <SelectContent>
      ...
      <div className="p-4 rounded-lg border bg-slate-50/50 space-y-4">
        <div className="flex items-center gap-2 text-sm font-medium text-slate-600">
          ...
          <span>Hình ảnh đại diện Đội ngũ</span>
        </div>
        <div className="flex items-center gap-6">
          ...
          <div className="space-y-2">
            ...
            <p className="text-[10px] text-slate-400">Đội ngũ có thể truy cập chung vào tài sản này.</p>
          </div>
        </div>
      </div>
