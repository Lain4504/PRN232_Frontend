      ...
      cell: ({ row }) => (
        <div className="flex items-center gap-6 py-4 transition-all duration-300">
          ...
          <div className="space-y-1">
            <div className="font-black text-slate-900 dark:text-white text-lg truncate max-w-[250px] leading-tight uppercase">{row.getValue("name")}</div>
      ...
      cell: ({ row }) => {
        ...
        return (
          <Badge variant="secondary" className={cn("rounded-lg px-2.5 py-0.5 font-black uppercase tracking-widest text-[9px] border-none",
            status === 'Active' ? 'bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400' : 'bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400'
          )}>
        ...
        <div className="flex items-center gap-3">
          <div className="flex -space-x-3">
            {[...Array(Math.min(3, row.original.membersCount || 0))].map((_, i) => (
              <div key={i} className="size-8 rounded-full bg-slate-100 dark:bg-slate-800 border-2 border-white dark:border-slate-900 flex items-center justify-center text-[10px] font-black text-slate-400 dark:text-slate-500 overflow-hidden">
                {String.fromCharCode(65 + i)}
              </div>
            ))}
            {(row.original.membersCount || 0) > 3 && (
              <div className="size-8 rounded-full bg-slate-900 dark:bg-primary border-2 border-white dark:border-slate-900 flex items-center justify-center text-[10px] font-black text-white shadow-sm">
                +{(row.original.membersCount || 0) - 3}
              </div>
            )}
          </div>
  ...
  return (
    <div className="space-y-12 pb-20 font-sans">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 border-b border-slate-100 dark:border-slate-800 pb-12 text-slate-900 dark:text-white transition-all duration-300">
        <div className="space-y-4">
          <div className="flex items-center gap-3">
            ...
            <span className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.3em]">Phối hợp & Cộng tác</span>
          </div>
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-6">
          <div className="p-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 shadow-sm flex items-center gap-4 transition-all duration-300">
            <div className="size-10 rounded-xl bg-slate-900 dark:bg-primary flex items-center justify-center text-white">
              <LayoutGrid className="size-5" />
            </div>
            <div className="space-y-0.5">
              <p className="text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Đội ngũ hiện có</p>
              <p className="text-sm font-black text-slate-900 dark:text-white uppercase tracking-tight">{rows.length} Nhóm</p>
            </div>
          </div>
      ...
      <div className="flex flex-col lg:flex-row items-center justify-between gap-6">
        ...
        {!checkFeatureAccess(profileType, 'teams') && (
          <div className="flex items-center gap-3 px-6 h-12 bg-amber-50 dark:bg-amber-500/10 rounded-2xl border border-amber-100 dark:border-amber-800 text-[10px] font-black text-amber-600 dark:text-amber-400 uppercase tracking-widest transition-all">
      ...
      <div className="relative">
        {isError ? (
          <div className="flex flex-col items-center justify-center py-32 text-center bg-rose-50/50 dark:bg-rose-900/10 rounded-3xl border border-dashed border-rose-200 dark:border-rose-800 transition-all">
            <AlertCircle className="size-16 text-rose-500 mb-6" />
            <h3 className="text-2xl font-black uppercase tracking-widest text-slate-900 dark:text-white">Lỗi hệ thống</h3>
            <p className="text-slate-500 dark:text-slate-400 font-medium max-w-sm mt-3 leading-relaxed">Không thể lấy dữ liệu đội nhóm. Vui lòng thử lại sau.</p>
          </div>
        ) : !checkFeatureAccess(profileType, 'teams') ? (
          <div className="flex flex-col items-center justify-center py-32 text-center bg-slate-50/50 dark:bg-slate-900/50 rounded-3xl border border-dashed border-slate-200 dark:border-slate-800 transition-all">
            <Shield className="size-16 text-slate-300 dark:text-slate-700 mb-8" />
            <h3 className="text-3xl font-black uppercase tracking-tight mb-3 text-slate-900 dark:text-white leading-none">Quyền truy cập hạn chế</h3>
            <p className="text-slate-500 dark:text-slate-400 font-medium max-w-sm mx-auto mb-10 leading-relaxed italic border-l-4 border-slate-100 dark:border-slate-800 pl-6">Quản lý đội nhóm nâng cao yêu cầu cấp độ quyền hạn cao hơn. Vui lòng nâng cấp gói dịch vụ để sử dụng tính năng này.</p>
            <Button variant="outline" className="h-12 px-8 rounded-xl font-black uppercase tracking-widest text-[10px] border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-900 dark:text-white hover:bg-slate-50 dark:hover:bg-slate-800">Xem bảng giá</Button>
          </div>
        ) : rows.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-32 px-6 text-center border border-dashed border-slate-200 dark:border-slate-800 rounded-3xl bg-slate-50/50 dark:bg-slate-900/50 transition-all duration-300">
            ...
            {!searchTerm && (
              <Button
                onClick={() => setOpenCreate(true)}
                className="h-14 px-10 rounded-2xl bg-slate-900 dark:bg-primary hover:bg-slate-800 dark:hover:bg-primary/90 text-white font-black uppercase tracking-widest text-[11px] shadow-2xl shadow-slate-200 dark:shadow-primary/20 transition-all hover:-translate-y-1"
              >
          ...
          <Card className="rounded-3xl border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-xl shadow-slate-200/40 dark:shadow-black/40 overflow-hidden relative group transition-all duration-300">
            <div className="absolute top-0 right-0 p-10 opacity-5 dark:opacity-10 -rotate-12 group-hover:rotate-0 transition-transform duration-1000 pointer-events-none">
              <Users className="size-40 text-slate-900 dark:text-primary" />
            </div>
            <CustomTable
              columns={columns}
              data={rows}
