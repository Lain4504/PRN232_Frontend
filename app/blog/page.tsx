            ...
            <main className="pt-40 pb-20 px-6">
                <div className="max-w-[1440px] mx-auto space-y-32">
                    ...
                    <section className="group cursor-pointer">
                        <div className="grid lg:grid-cols-2 bg-card/40 border border-border/40 rounded-[4rem] overflow-hidden backdrop-blur-3xl hover:bg-card/60 transition-all shadow-2xl">
                            ...
                            <div className="p-16 flex flex-col justify-between space-y-12">
                                <div className="space-y-6">
                                    <Badge className="bg-primary/10 text-primary border-none font-black text-[10px] px-6 py-2 uppercase tracking-[0.3em]">{posts[0].tag}</Badge>
                                    <h2 className="text-5xl font-black uppercase tracking-tighter leading-none">{posts[0].title}</h2>
                                    <p className="text-xl text-muted-foreground font-medium leading-relaxed">{posts[0].desc}</p>
                                </div>
                                <div className="flex items-center justify-between border-t border-border/20 pt-10">
                                    <div className="flex items-center gap-6">
                                        <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground/60">
                                            <Clock className="w-4 h-4" /> {posts[0].date}
                                        </div>
                                        <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.2em] text-primary">
                                            <User className="w-4 h-4" /> {posts[0].author}
                                        </div>
                                    </div>
                                    <Button variant="ghost" className="rounded-full h-16 w-16 border border-primary/20 group-hover:bg-primary group-hover:text-primary-foreground transition-all">
                    ...
                    <section className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
                        {posts.slice(1).map((post) => (
                            <div key={post.title} className="p-10 rounded-[3rem] bg-card/40 border border-border/40 backdrop-blur-3xl hover:bg-card/60 transition-all group flex flex-col justify-between h-[600px]">
                                <div className="space-y-6">
                                    ...
                                    <h3 className="text-3xl font-black uppercase tracking-tight leading-none group-hover:text-primary transition-colors">{post.title}</h3>
                                    <p className="text-muted-foreground font-medium text-sm leading-relaxed">{post.desc}</p>
                                </div>
                                <div className="flex items-center justify-between border-t border-border/10 pt-8 opacity-60">
